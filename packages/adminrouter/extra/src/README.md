...
automatically parse them into HTML documents. Please check the project
...
Additionally there is a (quick howto on basic tests
writing)[#how-to-write-tests]. Unfortunately it is impossible to fully cover
all aspects of the test harness, so it also may be helpful to look for examples
...
* `make lint` - launch linters which will check all the tests and test-harness
  files by default.
...
respectively. In the (How to write tests)[#how-to-write-tests] a rudimentary
