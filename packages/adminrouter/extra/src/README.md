...
There are two Admin Router "flavours" residing in DC/OS repos:
  * https://github.com/dcos/dcos/tree/master/packages/adminrouter - Opensource version or `Open` in short
  * https://github.com/mesosphere/dcos-enterprise/tree/master/packages/adminrouter - Enterprise version or `EE` in short

The `Open` version is the base on top of which `EE` version is build. `EE` is in
fact an overlay on top of `Open`, it re-uses some of its components.

### Complexity vs. code re-use
It is very important to understand that the more generalized the AR repositories
are, the more complex they will become. Increased complexity will result in
...
differentiation for sections can be avoided if we decide to use some more
...
└── server
    └── ee
        ...
        └── master.conf
```
All nginx related configuration directives reside in includes directory which
contain directories reflecting all the sections present in NGINX configuration
(http|main|server). Due to the fact that they are always present, no
...
and are present/included only in the Open AR repository. EE repo re-uses them
after being applied on top of Open repository. Contents of each of the
files is as follows:
...
directory is only present in EE repo, Open repo contains only `open` directories.
...
but it does not matter that much as:
 ...
 * the order of includes should not matter within particular section. This means
   that e.g. location blocks *MUST NOT* rely on the order in which they appear
   in the config file and define matching rules precise enough to avoid ambiguity.

All the includes are bound together by nginx.(master|slave).conf files.
`nginx.master.conf` for `Open` master looks like this ATM:
```
...
The EE repo contains only ee directories, all common/agent-common/master-common
...
remove all open directories from open repository before applying ee repo on
top of it.

This definitely is not a bulletproof solution for preventing code
...
a bit. It is sufficient to just differentiate lua code basing on the repo flavour.
...
* the same code is executed but with different call arguments. An example to
  this may be `auth.validate_jwt_or_exit()` function. In Open it takes no arguments,
 ...
 * modules which have flavour-specific function arguments export argument-less
   functions for the NGINX configuration. They translate the original call into
   a call with correct arguments. This approach requires splitting module into
   `ee.lua/open.lua/common.lua` parts which is described in next bulletpoint.
 ...
 * some libraries/functions are structured in a way that extracts parts common for
   ...
   auth.open or auth.ee modules as auth respectively.  Module is registered in
   global namespace so all other lua code uses it. This approach also allows
   for some degree of code separation enforcement as Open lua libs are removed
   during EE repo apply.

A special case of same code is executed but with different call arguments
problem is when the module needs to be initialized differently/passed
different arguments depending on the flavour. Great example of it is
`cache.lua` module which in Open does not require any extra
authentication data. In order to solve it, modules are required to provide
`.init()` function which is returned by require `<module-name>` stetement.
The init function accepts arguments which reconfigure module instance according
to the flavour requirements.

Modules that do not require any customization in regards of flavour are left
as-is/usually do not follow any of these patterns. It is possible to standardize
it but it does not seem to be justified.
