...
if($p.ExitCode -ne 0) {
    Throw ("Failed to install dcos-image Python library. Exit code: $($p.ExitCode)")
}

New-Item -ItemType "Directory" -Force -Path "$env:PKG_PATH\bin"
Copy-Item -Recurse -Path $PYTHON_PREFIX -Destination "$env:PKG_PATH\bin\"

New-Item -ItemType "Directory" -Force -Path "$env:PKG_PATH\etc"
Copy-Item -Path "$PKG_DIR\src\$env:PKG_NAME\LICENSE" -Destination "$env:PKG_PATH\etc\LICENSE"
Copy-Item -Path "$PKG_DIR\src\$env:PKG_NAME\NOTICE" -Destination "$env:PKG_PATH\etc\NOTICE"
