...
class OnpremLauncher(launch.util.AbstractLauncher):
    ...
    def create(self, config):
        return self.get_bare_cluster_launcher(config).create(config)

    def post_state(self, state):
        self.get_ssher(self.config).command(self.bootstrap_host, ['printf', state, '>', STATE_FILE])

    def get_last_state(self):
        return self.get_ssher(self.config).command(self.bootstrap_host, ['cat', STATE_FILE]).decode().strip()

    def get_bare_cluster_launcher(self, info):
        if info['platform'] == 'aws':
            return launch.aws.BareClusterLauncher(info)
        else:
            raise launch.util.LauncherError('Platform currently not supported for onprem: {}'.format(
                info['platform']))

    def get_onprem_cluster(self, info):
        return test_util.onprem.OnpremCluster.from_hosts(
            ssher=self.get_ssher(info),
            hosts=self.get_bare_cluster_launcher(info).get_hosts(info),
            num_masters=int(info['num_masters']),
            num_private_agents=int(info['num_private_agents']),
            num_public_agents=int(info['num_public_agents']))

    def get_completed_onprem_config(self, cluster, info):
        onprem_config = yaml.load(info['onprem_dcos_config_contents'])
        ...
        if onprem_config.get('ssh_key') == 'unset':
            onprem_config['ssh_key'] = info['ssh_private_key']
    ...
    def wait(self, info):
        ...
        self.get_bare_cluster_launcher(info).wait(info)
        cluster = self.get_onprem_cluster(info)
        self.bootstrap_host = cluster.bootstrap_host.public_ip
        log.info('Waiting for SSH connectivity to bootstrap host...')
        check_ssh(self.get_ssher(info), self.bootstrap_host, info['ssh_port'])
        try:
            self.get_ssher(info).command(self.bootstrap_host, ['test', '-f', STATE_FILE])
            last_complete = self.get_last_state()
        except subprocess.CalledProcessError:
            last_complete = None

        if last_complete is None:
            cluster.setup_installer_server(info['installer_url'], False)
            last_complete = 'SETUP'
            self.post_state(last_complete)

        installer = test_util.onprem.DcosInstallerApiSession(Url(
            'http', self.bootstrap_host, '', '', '', info['installer_port']))
        if last_complete == 'SETUP':
            last_complete = 'GENCONF'
            installer.genconf(self.get_completed_onprem_config(cluster, info))
            self.post_state(last_complete)
    ...
    def delete(self, info):
        ...
        """
        self.get_bare_cluster_launcher(info).delete(info)
