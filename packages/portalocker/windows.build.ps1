...
$params = @("install", "--no-deps", "--no-index", "--prefix=$PYTHON_PREFIX", "$PKG_DIR\src\portalocker\portalocker-1.2.1-py2.py3-none-any.whl")
$p = Start-Process -Wait -PassThru -FilePath "pip.exe" -ArgumentList $params
if($p.ExitCode -ne 0) {
    Throw ("Failed to install portalocker Python library. Exit code: $($p.ExitCode)")
}
