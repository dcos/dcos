...
def test_metrics_ping(cluster):
    ...
    for agent in cluster.slaves:
        ...
        'from private agent.'
        assert response.json()['ok'], 'Metrics /ping is not ok on private '
        'agent.'
    ...
    for agent in cluster.public_slaves:
        ...
        assert response.status_code == 200, 'Got non-200 status code '
        'from public agent.'
        assert response.json()['ok'], 'Metrics /ping is not ok on public agent.'

    for master in cluster.masters:
        response = cluster.metrics.get('ping', node=master)
        assert response.status_code == 200, 'Got non-200 status code '
        'from public agent.'
        assert response.json()['ok'], 'Metrics /ping is not ok on master.'
