...
# Expected latency for all dcos-diagnostics units to refresh after postflight
# plus another minute to allow for check-time to settle. See: DCOS_OSS-988
LATENCY = 120
...
def test_dcos_diagnostics_health(dcos_api_session):
    ...
    # Check all masters dcos-diagnostics instances on base port since this is
    # extra-cluster request (outside localhost)
    for host in dcos_api_session.masters:
        ...
        assert isinstance(response['units'],
                          list), 'units field must be a list'
        ...
        for unit in response['units']:
            ...
            for required_field_unit in required_fields_unit:
                assert required_field_unit in unit, \
                    '{} must be in a unit repsonse'
    ...
    # Check all agents running dcos-diagnostics behind agent-adminrouter
    # on 61001
    for host in dcos_api_session.slaves:
        ...
        for unit in response['units']:
            ...
            for required_field_unit in required_fields_unit:
                assert required_field_unit in unit, \
                    '{} must be in a unit repsonse'.format(
                        required_field_unit)
...
def test_dcos_diagnostics_nodes(dcos_api_session):
    ...
    for master in dcos_api_session.masters:
        ...
        assert len(
            response) == 1, 'nodes response must have only one field: nodes'
        assert 'nodes' in response
        assert isinstance(response['nodes'], list)
        assert len(response['nodes']) == \
            len(dcos_api_session.masters + dcos_api_session.all_slaves), \
            ('a number of nodes in response must be {}'.format(
                len(dcos_api_session.masters + dcos_api_session.all_slaves)))
    ...
    for master in dcos_api_session.masters:
        ...
        for node in nodes:
            node_response = check_json(
                dcos_api_session.health.get('/nodes/{}'.format(node),
                                            node=master))
...
def test_dcos_diagnostics_nodes_node_units(dcos_api_session):
    """
    test a list of units from a specific node,
    endpoint /system/health/v1/nodes/<node>/units
    """
    for master in dcos_api_session.masters:
        # get a list of nodes
        response = check_json(
        ...
        for node in nodes:
            units_response = check_json(
                dcos_api_session.health.get('/nodes/{}/units'.format(node),
                                            node=master))

            assert len(units_response,
...
def test_dcos_diagnostics_nodes_node_units_unit(dcos_api_session):
    """
    test a specific unit for a specific node,
    endpoint /system/health/v1/nodes/<node>/units/<unit>
    """
    for master in dcos_api_session.masters:
        response = check_json(
            dcos_api_session.health.get('/nodes', node=master))
...
def test_systemd_units_health(dcos_api_session):
    """
    test all units and make sure the units are healthy. This test will fail if
    any of systemd unit is unhealthy, meaning it focuses on making sure the
    dcos_api_session is healthy, rather then testing dcos-diagnostics itself.
    """
    unhealthy_output = []
    assert dcos_api_session.masters, "Must have at least 1 master node"
    report_response = check_json(
        dcos_api_session.health.get('/report',
    ...
    for unit_name, unit_props in report_response['Units'].items():
        ...
        if unit_props['Health'] != 0:
            assert 'Nodes' in unit_props, \
                "Unit {} missing `Nodes` field".format(unit_name)
            assert isinstance(unit_props['Nodes'], list), \
                'Field `Node` must be a list'
            for node in unit_props['Nodes']:
                assert 'Health' in node, \
                    'Field `Health` is expected to be in '
                'nodes properties, got {}'.format(node)
                if node['Health'] != 0:
                    assert 'Output' in node, \
                        'Field `Output` is expected to bein nodes properties, '
                    'got {}'.format(node)
                    assert isinstance(node['Output'], dict), \
                        'Field `Output` must be a dict'
                    assert unit_name in node['Output'], \
                        'unit {} must be in node Output, got {}'.format(
                            unit_name, node['Output'])
                    assert 'IP' in node, 'Field `IP` is expected to be in '
                    'nodes properties, got {}'.format(node)
                    unhealthy_output.append(
                        'Unhealthy unit {} has been found on node {}, health '
                        'status {}. journalctl output {}'.format(
...
def test_dcos_diagnostics_units_unit(dcos_api_session):
    """
    test a unit response in a right format,
    endpoint: /system/health/v1/units/<unit>
    """
    for master in dcos_api_session.masters:
        units_response = check_json(
            dcos_api_session.health.get('/units', node=master))
        ...
        for unit in pulled_units:
            unit_response = check_json(
                dcos_api_session.health.get('/units/{}'.format(unit),
                                            node=master))
...
def test_dcos_diagnostics_units_unit_nodes(dcos_api_session):
    """
    test a list of nodes for a specific unit,
    endpoint /system/health/v1/units/<unit>/nodes
    """
    def get_nodes_from_response(response):
        assert 'nodes' in response, \
               'response must have field `nodes`. Got {}'.format(response)
        nodes_ip_map = make_nodes_ip_map(dcos_api_session)
        nodes = []
        for node in response['nodes']:
            assert 'host_ip' in node, \
                   'node response must have `host_ip` field. '
            'Got {}'.format(node)
            assert node['host_ip'] in nodes_ip_map, \
                'nodes_ip_map must have node {}. '
            'Got {}'.format(node['host_ip'], nodes_ip_map)
    ...
    for master in dcos_api_session.masters:
        ...
        for unit in pulled_units:
            nodes_response = check_json(
                dcos_api_session.health.get('/units/{}/nodes'.format(unit),
                                            node=master))
            validate_node(nodes_response['nodes'])

        # make sure dcos-mesos-master.service has master nodes and
        # dcos-mesos-slave.service has agent nodes
        ...
        assert set(master_nodes) == set(
            dcos_api_session.masters), 'a list of difference: {}'.format(
        ...
        assert len(agent_nodes) == len(
            dcos_api_session.slaves), '{} != {}'.format(
                agent_nodes, dcos_api_session.slaves)


@pytest.mark.supportedwindows
def test_dcos_diagnostics_units_unit_nodes_node(dcos_api_session):
    """
    test a specific node for a specific unit,
    endpoint /system/health/v1/units/<unit>/nodes/<node>
    """
    ...
    for master in dcos_api_session.masters:
        ...
        for unit in pulled_units:
            ...
            for node in pulled_nodes:
                check_path = '/units/{}/nodes/{}'.format(unit, node)
                node_response = check_json(
                    dcos_api_session.health.get(check_path, node=master))
                assert len(node_response) == len(
                    required_node_fields), 'required fields: {}'.format(
                        ', '.format(required_node_fields))

                for required_node_field in required_node_fields:
                    assert required_node_field in node_response, \
                        'field {} must be set'.format(required_node_field)

                # host_ip, health, role, help cannot be empty
                assert node_response[
                    'host_ip'], 'host_ip field cannot be empty'
                assert node_response['health'] in [0,
                                                   1], 'health must be 0 or 1'
...
def test_dcos_diagnostics_bundle_create_download_delete(
        dcos_api_session, use_legacy_api):
    """
...
def _check_diagnostics_bundle_status(dcos_api_session):
    ...
    required_status_fields = [
        ...
        'diagnostics_job_timeout_min',
        'journald_logs_since_hours',
        'diagnostics_job_get_since_url_timeout_min',
        'command_exec_timeout_sec',
        'diagnostics_partition_disk_usage_percent',
    ...
    for _, properties in diagnostics_bundle_status.items():
        for required_status_field in required_status_fields:
            assert required_status_field in properties, \
                'property {} not found'.format(required_status_field)
...
def _download_and_extract_bundle_from_another_master(
        dcos_api_session,
        bundle,
        diagnostics,
):
...
def _download_bundle_from_master(dcos_api_session, master_index, bundle,
    ...
    expected_common_files = [
        ...
        'optmesospherebincurl_-s_-S_http:localhost:62080v1metricsdefault.output',  # NOQA
        'optmesospherebincurl_-s_-S_http:localhost:62080v1metricsdns.output',
        'optmesospherebincurl_-s_-S_http:localhost:62080v1metricsmesos_listener.output',  # NOQA
        'optmesospherebincurl_-s_-S_http:localhost:62080v1metricslashup.output',  # NOQA
        'timedatectl.output',
    ...
    expected_master_files = [
        'binsh_-c_cat proc`systemctl show dcos-mesos-master.service -p MainPID| cut -d\'=\' -f2`environ.output',  # NOQA
        '5050-quota.json',
    ...
    expected_agent_files = [
        ...
        'binsh_-c_cat proc`systemctl show dcos-mesos-slave.service -p MainPID| cut -d\'=\' -f2`environ.output',  # NOQA
    ] + expected_agent_common_files + expected_common_files

    # for public agent host
    expected_public_agent_files = [
        'dcos-mesos-slave-public.service',
        'binsh_-c_cat proc`systemctl show dcos-mesos-slave-public.service -p MainPID| cut -d\'=\' -f2`environ.output',  # NOQA
    ] + expected_agent_common_files + expected_common_files
    ...
    def _get_dcos_diagnostics_health(z: zipfile.ZipFile, item: str):
        # try to load dcos-diagnostics health report and validate the report
        # is for this host
        try:
            _health_report = _read_from_zip(z, item)
        except KeyError:
            # we did not find a key in archive, let's take a look at items in
            # archive and try to read
            # diagnostics logs.
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for master_ip in dcos_api_session.masters:
            ...
            # try to load dcos-diagnostics health report and validate the
            # report is for this host
            ...
            # make sure systemd unit output is correct and does not contain
            # error message
...
def get_file_content(unzipped_file, z):
    ...
    raise AssertionError("Not found {} nor {} in {}".format(
        unzipped_file, expected_gzipped_file, archived_items))
...
def validate_node(nodes):
    ...
    for node in nodes:
        ...
        for required_field in required_fields:
            assert required_field in node, \
                '{} must be in node. Actual: {}'.format(required_field, node)
...
def validate_units(units):
    ...
    for unit in units:
        ...
        for required_field in required_fields:
            assert required_field in unit, \
                'unit response must have field: {}. Actual: {}'.format(
                    required_field, unit)
...
def validate_unit(unit):
    ...
    for required_field in required_fields:
        assert required_field in unit, \
            '{} must be in a unit. Actual: {}'.format(required_field, unit)
...
def validate_state(state_output):
    ...
    assert len(state["frameworks"]) > 1, \
        "bundle must contain information about frameworks"
...
def verify_archived_items(folder, archived_items, expected_files):
    for expected_file in expected_files:
        ...
        # https://jira.mesosphere.com/browse/DCOS_OSS-4531
        ...
        message = ('expecting {} or {} in {}'.format(expected_file,
                                                     expected_gzipped_file,
                                                     archived_items))
