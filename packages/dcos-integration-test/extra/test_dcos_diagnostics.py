...
def test_diagnostics_bundle_contains_marathon_apps(dcos_api_session):
    """
    This test confirms that the dcos-diagnostics bundle contains
    detailed content generated by Marathon's /v2/apps endpoint.
    """

    # Launch a single app.
    app, _ = marathon_test_app()
    with dcos_api_session.marathon.deploy_and_cleanup(app, check_health=False):
        # Initiate bundle creation.
        create_bundle_response = check_json(
            dcos_api_session.health.post('/report/diagnostics/create', json={"nodes": ["all"]}))
        bundle_name = create_bundle_response['extra']['bundle_name']
        ...
        wait_for_diagnostics_job(dcos_api_session, last_datapoint)
        wait_for_diagnostics_list(dcos_api_session)
        # Download the bundle zipfile to a temporary directory.
        with tempfile.TemporaryDirectory() as tmp_dir:
            bundle_full_location = os.path.join(tmp_dir, bundle_name)
            with open(bundle_full_location, 'wb') as f:
                r = dcos_api_session.health.get(
                    os.path.join('/report/diagnostics/serve', bundle_name),
                    stream=True,
                    node=dcos_api_session.masters[0],
                )
                for chunk in r.iter_content(1024):
                    f.write(chunk)
            # Destroy bundle again, so we don't leave the cluster in a weird state.
            delete_bundle_response = dcos_api_session.health.post(
                '/report/diagnostics/delete/{}'.format(bundle_name))
            delete_bundle_response.raise_for_status()
            # validate bundle zip file.
            assert zipfile.is_zipfile(bundle_full_location)
            z = zipfile.ZipFile(bundle_full_location)
            # Read the Marathon /v2/apps output from the first master.
            master_folder = dcos_api_session.masters[0] + '_master/'
            v2_apps_filename = master_folder + expanded_config['marathon_port'] + '-v2_apps.json.gz'
            gzipped_contents = z.open(v2_apps_filename)
            contents = gzip.decompress(gzipped_contents.read())
            logging.debug("File {} had contents: {}".format(v2_apps_filename, contents))
            apps = json.loads(contents)
            # Ensure that the /v2/apps endpoint output captured in the dcos
            # diagnostics bundle contains details for the sleeper
            # application that we launched in this test.
            had_sleeper = False
            for app in apps['apps']:
                if app['id'] == app['id']:
                    had_sleeper = True
            assert had_sleeper, "Could not find {} in the /v2/apps endpoint output".format(app['id'])
