...
def test_if_ucr_app_can_be_deployed_with_image_whiteout(dcos_api_session):
    ...
    app = {
        ...
        'healthChecks': [{
            ...
            'command': {
                'value':
                'test ! -f /dir1/file1 && test ! -f /dir1/dir2/file2 && test -f /dir1/dir2/file3',  # NOQA
            },
...
def test_if_ucr_app_can_be_deployed_with_image_digest(dcos_api_session):
    ...
    app = {
        ...
        'container': {
            ...
            'docker': {
                'image':
                'library/alpine@sha256:9f08005dff552038f0ad2f46b8e65ff3d25641747d3912e3ea8da6785046561a',  # NOQA
            },
        },
        'healthChecks': [{
            'protocol': 'COMMAND',
...
def test_if_ucr_app_can_be_deployed_with_auto_cgroups(dcos_api_session):
    ...
    app = {
        ...
        'healthChecks': [{
            ...
            'command': {
                'value':
                'test `cat /sys/fs/cgroup/memory/memory.soft_limit_in_bytes` = 67108864 && '  # NOQA
...
def test_if_ucr_app_can_be_deployed_with_shm_in_specified_size(
        dcos_api_session):
    """Marathon app deployment integration test using the Mesos Containerizer.
    ...
    app = {
        ...
        'healthChecks': [{
            'protocol': 'COMMAND',
            'command': {
                'value': 'df -m /dev/shm | grep -w 1234',
            },
            'gracePeriodSeconds': 5,
            'intervalSeconds': 10,
            'timeoutSeconds': 10,
...
def test_if_ucr_pods_can_be_deployed_with_auto_cgroups(dcos_api_session):
    ...
    (i.e., 0.3 * 1024 = 307 CPU shares) and 96MB memory
    (i.e., 100663296 bytes).
    """
    test_uuid = uuid.uuid4().hex
    pod_definition = {
        ...
        'containers': [
            {
                ...
                'exec': {
                    'command': {
                        'shell':
                        'test `cat /sys/fs/cgroup/memory/memory.soft_limit_in_bytes` = 100663296 && '  # NOQA
...
def test_if_ucr_pods_can_share_shm_with_childs(dcos_api_session):
    ...
    pod_definition = {
        ...
        'containers': [{
            ...
            'exec': {
                'command': {
                    'shell': 'df -m /dev/shm | grep -w 1234',
                },
            },
            'linuxInfo': {
                'ipcInfo': {
            ...
            'exec': {
                'command': {
                    'shell':
                    'test -z $(df -m /dev/shm | grep -w 1234) && '
                    'echo $PING > foo; while true; do sleep 1; done',
                },
            },
            'healthcheck': {
