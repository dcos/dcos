...
def validate_json_entry(entry: dict):
    ...
    assert set(entry.keys()) <= required_fields, (
        "Entry didn't have all required fields. Entry fields: {}, "
        "required fields:{}".format(entry, required_fields))
...
def test_log_text(dcos_api_session):
    for node in dcos_api_session.masters + dcos_api_session.all_slaves:
        ...
        assert entries_count == 10, \
            'Expect 10 log entries. Got {}. All lines {}'.format(
                entries_count, logs)
...
def test_log_proxy(dcos_api_session):
    ...
    for slave_id in slaves_ids:
        response = dcos_api_session.get(
            '/system/v1/agent/{}/logs/v1/range/?skip_prev=10&limit=10'.format(
                slave_id))
        check_response_ok(response, {'Content-Type': 'text/plain'})
        lines = list(filter(lambda x: x != '', response.text.split('\n')))
        assert len(
            lines) == 10, 'Expect 10 log entries. Got {}. All lines {}'.format(
                len(lines), lines)
...
def test_task_logs(dcos_api_session):
    ...
    with dcos_api_session.marathon.deploy_and_cleanup(task_definition,
                                                      check_health=False):
        url = get_task_url(dcos_api_session, task_id)
        check_response(
            'STDOUT_LOG',
            lambda: dcos_api_session.get(url + '?filter=STREAM:STDOUT'))
        check_response(
            'STDERR_LOG',
            lambda: dcos_api_session.get(url + '?filter=STREAM:STDERR'))
...
def test_pod_logs(dcos_api_session):
    ...
    pod_definition = {
        ...
        'containers': [{
            ...
            'exec': {
                'command': {
                    'shell':
                    'echo $PING > foo;echo STDOUT_LOG;echo STDERR_LOG >&2;sleep 10000',  # NOQA
                },
            },
            'resources': {
                'cpus': 0.1,
    ...
    with dcos_api_session.marathon.deploy_pod_and_cleanup(pod_definition):
        ...
        response = dcos_api_session.get(url + '/download',
                                        query='limit=10&postfix=stdout')
...
def check_response(content, get_response):
    ...
    check_response_ok(response, {})
    assert content in response.text, 'Missing {} in response {}'.format(
...
def get_task_url(dcos_api_session, task_name, stream=False):
    ...
    assert 'frameworks' in state_response_json, \
        'Missing field `framework` in {}'.format(
            state_response_json)
    assert isinstance(state_response_json['frameworks'], list), \
    ...
    for framework in state_response_json['frameworks']:
        assert 'name' in framework, \
            'Missing field `name` in `frameworks`. Got {}'.format(
                state_response_json)
        ...
        assert isinstance(
            framework['tasks'],
            list), '`tasks` must be list. Got {}'.format(state_response_json)
        for task in framework['tasks']:
            ...
            assert 'framework_id' in task, \
                'Missing `framework_id` in task. Got {}'.format(
                    state_response_json)
            assert 'executor_id' in task, \
                'Missing `executor_id` in task. Got {}'.format(
                    state_response_json)
            assert 'id' in task, 'Missing `id` in task. Got {}'.format(
                state_response_json)
            assert 'slave_id' in task, \
                'Missing `slave_id` in task. Got {}'.format(
                    state_response_json)
            ...
            assert isinstance(statuses,
                              list), 'Invalid field `statuses`. Got {}'.format(
                                  state_response_json)
            assert len(
                statuses,
    ...
    return '/system/v1/agent/{}/logs/v1/{}/framework/{}/executor/{}/container/{}'.format(  # NOQA
...
def test_log_v2_text(dcos_api_session):
    for node in dcos_api_session.masters + dcos_api_session.all_slaves:
        response = dcos_api_session.logs.get('/v2/component?limit=10',
                                             node=node)
        check_response_ok(response, {'Content-Type': 'text/plain'})
...
def test_log_v2_proxy(dcos_api_session):
    ...
    for slave_id in slaves_ids:
        response = dcos_api_session.get(
            '/system/v1/agent/{}/logs/v2/component?skip=-10&limit=10'.format(
                slave_id))
        check_response_ok(response, {'Content-Type': 'text/plain'})
        lines = list(filter(lambda x: x != '', response.text.split('\n')))
        assert len(
...
def test_log_v2_task_logs(dcos_api_session):
    ...
    with dcos_api_session.marathon.deploy_and_cleanup(task_definition,
        ...
        check_response(
            'STDOUT_LOG',
            lambda: dcos_api_session.logs.get('/v2/task/{}/file/stdout'.format(
                task_id)),
        )
        check_response(
            'STDERR_LOG',
            lambda: dcos_api_session.logs.get('/v2/task/{}/file/stderr'.format(
                task_id)),
        )
        _assert_files_in_browse_response(dcos_api_session, task_id,
                                         ['stdout', 'stderr'])
        _assert_can_download_files(dcos_api_session, task_id,
                                   ['stdout', 'stderr'])
...
def test_log_v2_pod_logs(dcos_api_session):
    ...
    with dcos_api_session.marathon.deploy_pod_and_cleanup(pod_definition):
        check_response('STDOUT_LOG',
                       lambda: dcos_api_session.logs.get('/v2/task/sleep1'))
        check_response(
            'STDERR_LOG',
            lambda: dcos_api_session.logs.get('/v2/task/sleep1/file/stderr'))
        _assert_files_in_browse_response(dcos_api_session, pod_id,
                                         ['stdout', 'stderr', 'foo'])
        _assert_can_download_files(dcos_api_session, pod_id,
                                   ['stdout', 'stderr', 'foo'])
...
def test_log_v2_api(dcos_api_session):
    ...
    with dcos_api_session.marathon.deploy_and_cleanup(task_definition,
        ...
        # set cursor to 7 (bytes) which the second word and skip -1 lines and
        # limit 1
        response = dcos_api_session.logs.get(
        ...
        # https://jira.mesosphere.com/browse/DCOS_OSS-1995
        response = dcos_api_session.logs.get(
            '/v2/task/{}/file/test?cursor=END&skip=-5'.format(task_id))
        check_response_ok(response, {})
        assert response.text == "one\ntwo\nthree\nfour\nfive\n"

        # make sure if 'Last-Event-ID' header is passed, other get parameters
        # are ignored https://jira.mesosphere.com/browse/DCOS_OSS-2292
    ...
    for item in data:
        for field in expected_fields:
            assert field in item, \
                'Field {} must be in response. Item {}'.format(
                    field, item)
