...
# Creates and yields the initial ATTACH_CONTAINER_INPUT message, then a data
# message, then an empty data chunk to indicate end-of-stream.
...
def test_if_marathon_app_can_be_debugged(dcos_api_session):
    ...
    # Cannot use deploy_and_cleanup because we must attach to a running
    # app/task/container.
    ...
    with dcos_api_session.marathon.deploy_and_cleanup(app):
        ...
        assert container_id is not None, \
            'Container ID not found for instance of app_id {}'.format(
                app_id)
        assert agent_id is not None, \
            'Agent ID not found for instance of app_id {}'.format(
                app_id)
        ...
        assert agent_hostname is not None, \
            'Agent hostname not found for agent_id {}'.format(
                agent_id)
        ...
        launch_output = _post_agent('/api/v1',
                                    output_headers,
                                    json=lncs_data,
                                    stream=True)
        ...
        attached_output = _post_agent('/api/v1',
                                      output_headers,
...
def test_files_api(dcos_api_session):
    '''
    This test verifies that the standard output and error of a Mesos task can
    be read. We check that neither standard output nor error are empty files.
    Since the default `marathon_test_app()` does not write to its standard
    output the task definition is modified to output something there.
    '''
    app, test_uuid = test_helpers.marathon_test_app()
...
def test_if_ucr_app_runs_in_new_pid_namespace(dcos_api_session):
    ...
    with dcos_api_session.marathon.deploy_and_cleanup(app, check_health=False):
        ...
        assert len(get_ps_output().split()) <= 4, \
            'UCR app has more than 4 processes running in its pid namespace'
...
def test_containerizer_debug_endpoint(dcos_api_session):
    # Test that we can poll `/containerizer/debug` endpoint exposed by the
    # agent.
...
def test_blkio_stats(dcos_api_session):
    ...
    if expanded_config['provider'] == 'azure' or expanded_config[
            'platform'] == 'azure':
        pytest.skip('See: https://jira.mesosphere.com/browse/DCOS-49023')

    # Launch a Marathon application to do some disk writes, and then verify
    # that the cgroups blkio statistics of the application can be correctly
    # retrieved.
    app, test_uuid = test_helpers.marathon_test_app(
        container_type=marathon.Container.MESOS)
    app_id = 'integration-test-{}'.format(test_uuid)
    ...
    with dcos_api_session.marathon.deploy_and_cleanup(app, check_health=False):
        ...
        for stat in stats:
            # Find the statistic for the Marathon application that we deployed.
            # Since what that Marathon application launched is a Mesos command
            # task (i.e., using Mesos built-in command executor), the executor
            # ID will be same as the task ID, so if we find the `app_id` in an
            # executor ID of a statistic, that must be the statistic entry
            # corresponding to the application that we deployed.
            if app_id in stat['executor_id']:
                # We only care about the blkio throttle statistics but not the
                # blkio cfq statistics, because in the environment where the
                # disk IO scheduler is not `cfq`, all the cfq statistics may be
                # 0.
        ...
        # We expect the statistics retrieved from Mesos agent are equal or
        # greater than what we
        # did with the `dd` command (i.e., 10 and 10240), because:
        #   1. Besides the disk writes done by the `dd` command, the statistics
        #      may also include some disk reads, e.g., to load the necessary
        #      executable binary and libraries.
        #   2. In the environment where RAID is enabled, there may be multiple
        #      disk writes to different disks for a single `dd` write.
...
def test_fault_domain(dcos_api_session):
    ...
    for agent in state['slaves']:
        ...
        assert agent_region == expected_region, \
            'expect region {}. Got {}'.format(
                expected_region, agent_region)

        # agent_zone might be different on agents, so we just make sure it's a
        # sane value
...
def reserved_disk(dcos_api_session):
    ...
    try:
        ...
        # Create a unique role to reserve the disk to. The test framework
        # should register in this role.
        ...
        # Reserve the remaining agent resources for another role. We let the
        # Mesos master perform the calculation of the unreserved resources on
        # the agent which requires another query.
        r = dcos_api_session.get('/mesos/slaves')
        assert r.status_code == 200, r.text
        response = json.loads(r.text)
