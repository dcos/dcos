...
def split_by_token(token_prefix, token_suffix, string, strip_token_decoration=False):
    ...
    def _raise_exception_if_suffix_in(substring):
        if token_suffix in substring:
            logging.debug("Token suffix found without matching prefix in string: {}".format(repr(string)))
    ...
    while num_chars_consumed < len(string):
        # Find the next token.
        token_start = _next_substring(string, token_prefix, num_chars_consumed)
        if not token_start:
            # No token found. Yield the rest of the string and return.
            remainder = string[num_chars_consumed:]
        ...
        if token_start[0] > num_chars_consumed:
            preceding_string = string[num_chars_consumed:token_start[0]]
            _raise_exception_if_suffix_in(preceding_string)
            yield preceding_string, False

        # Find the end of the token.
        token_end = _next_substring(string, token_suffix, token_start[1])
        if not token_end or token_prefix in string[token_start[1]:token_end[0]]:
            # Can't find a closing suffix, or found two consecutive prefixes without a suffix between them.
            logging.debug("Token prefix found without matching suffix in string: {}".format(repr(string)))
            raise Exception("Token prefix found without matching suffix")

        # Yield the token.
        if strip_token_decoration:
            # Omit the token's prefix and suffix.
            yield string[token_start[1]:token_end[0]], True
        else:
            # Yield the entire token.
            yield string[token_start[0]:token_end[1]], True
