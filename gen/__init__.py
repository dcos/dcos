...
def render_late_content(content, late_values):
    ...
    def _next_substring(string, substring, start=0):
        idx = string.find(substring, start)
        if idx < 0:
            return None
        return idx, idx + len(substring)

    def _interpolate_values(string):
        if not string:
            return

        # Find the next placeholder.
        placeholder_start = _next_substring(string, gen.internals.LATE_BIND_PLACEHOLDER_START)
        ...
        if placeholder_start[0] > 0:
            yield string[:placeholder_start[0]]

        # Find the end of the placeholder.
        placeholder_end = _next_substring(string, gen.internals.LATE_BIND_PLACEHOLDER_END, placeholder_start[1])
        if not placeholder_end:
            log.debug("Can't find end of placeholder in string: {}".format(repr(string)))
        ...
        name = string[placeholder_start[1]:placeholder_end[0]]
        if name not in late_values:
            log.debug('Found placeholder for unknown value "{}" in string: {}'.format(name, repr(string)))
        ...
        yield from _interpolate_values(string[placeholder_end[1]:])
