...
@pytest.mark.skipif(is_windows, reason="dcos-signal not ported to Windows")
...
@pytest.mark.skipif(is_windows, reason="No exhibitor component on Windows")
...
@pytest.mark.skipif(is_windows, reason="No UI component on Windows")
...
@pytest.mark.skipif(is_windows, reason="No Mesos DNS on Windows")
...
@pytest.mark.skipif(is_windows, reason="No exhibitor on Windows")
...
@pytest.mark.skipif(is_windows, reason="No overlay networks on Windows")
...
def test_fault_domain_disabled():
    if is_windows:
        fault_domain_script = 'fault-domain-detect/aws.ps1'
    else:
        fault_domain_script = 'fault-domain-detect/aws.sh'
    arguments = make_arguments(new_arguments={
        'fault_domain_detect_filename': pkg_resources.resource_filename('gen', fault_domain_script)
    })
...
def test_edited_ip_detect_script_yields_new_packages():
    ...
    arguments = make_arguments(new_arguments={'ip_detect_filename': filename})
    try:
